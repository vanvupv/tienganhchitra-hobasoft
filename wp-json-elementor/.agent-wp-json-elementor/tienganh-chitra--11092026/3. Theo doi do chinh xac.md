...
| **Section 3** | **Khối Tabs & Posts Lưới Học Viên Động** | Boxed `1240px`, Widget `nested-tabs` & `posts` | **100%** | **Chuẩn kiến trúc Tab & Post:** H2 tiêu đề + Widget `nested-tabs` (4 Tabs: IC3, Cambridge, IELTS, Giải quốc tế). Mỗi Tab chứa bên trong: 1) Header Card giới thiệu chứng chỉ; 2) Widget `posts` động lấy từ CPT `hoc_vien` (Lưới 4 cột PC / 2 cột TB / 1 cột MB, có phân trang số `< 1 2 3 >`). |
| **Section 4** | **Những Con Số Ấn Tượng** | Boxed `1240px`, Thẻ bo góc `18px`, nền `#F8FAFC` | **100%** | H3 `NHỮNG CON SỐ ẤN TƯỢNG` + 4 khối icon box thống kê: `150+` (IC3), `80+` (GS5+), `30+` (Giải quốc tế), `100%` (Vinh danh). |
| **Section 5** | **Call To Action Tuyển Sinh** | Boxed `1240px`, Gradient `#EBF5FF` - `#E0F2FE`, bo góc `20px` | **100%** | Cột trái: Icon users tròn + H3 `Cùng con tự tin chinh phục tiếng Anh` + Nút `Đăng ký ngay →`. Cột phải: Ảnh học viên mỉm cười. |
| **TỔNG THỂ** | **TOÀN TRANG BẢNG VÀNG HỌC VIÊN (KIẾN TRÚC TAB & POST)** | **100% Flexbox Elementor Pro 3.x Native** | **100% / 100%** | **Sẵn sàng import trực tiếp qua Elementor Templates!** |
